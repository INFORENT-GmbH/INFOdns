...
export async function authRoutes(app: FastifyInstance) {
  ...
  app.get('/auth/invites', { preHandler: requireAdmin }, async () => {
    type Row = { id: number; email: string; first_name: string; last_name: string; role: string; locale: string; tenant_ids: string | null; expires_at: string; created_at: string }
    const rows = await query(
      'SELECT id, email, first_name, last_name, role, locale, tenant_ids, expires_at, created_at FROM user_invites WHERE used_at IS NULL AND expires_at > NOW() ORDER BY created_at DESC'
    ) as Row[]
    return rows.map(r => ({
      ...r,
  ...
  app.post('/auth/invite', { preHandler: requireAdmin }, async (req, reply) => {
    ...
    await execute(
      'INSERT INTO user_invites (email, token_hash, role, first_name, last_name, locale, tenant_ids, invited_by, expires_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
      [
        body.data.email, tokenHash, body.data.role, body.data.first_name, body.data.last_name,
    ...
    execute(
      ...
      [body.data.email, JSON.stringify({
        ...
        full_name: [body.data.first_name, body.data.last_name].filter(Boolean).join(' '),
        inviteUrl,
      })]
  ...
  app.get<{ Params: { token: string } }>('/auth/invite/:token', async (req, reply) => {
    ...
    type InviteRow = { email: string; first_name: string; last_name: string; role: string; locale: string; expires_at: string; used_at: string | null }
    const invite = await queryOne<InviteRow>(
      'SELECT email, first_name, last_name, role, locale, expires_at, used_at FROM user_invites WHERE token_hash = ?',
      [tokenHash]
    )
    ...
    return { email: invite.email, first_name: invite.first_name, last_name: invite.last_name, role: invite.role, locale: invite.locale }
  })
  ...
  app.post('/auth/accept-invite', async (req, reply) => {
    ...
    type InviteRow = { id: number; email: string; first_name: string; last_name: string; role: 'admin' | 'operator' | 'tenant'; locale: 'en' | 'de'; tenant_ids: string | null; expires_at: string; used_at: string | null }
    const invite = await queryOne<InviteRow>(
      'SELECT id, email, first_name, last_name, role, locale, tenant_ids, expires_at, used_at FROM user_invites WHERE token_hash = ?',
      [tokenHash]
    )
    if (!invite) return reply.status(404).send({ code: 'INVITE_NOT_FOUND' })
    if (invite.used_at) return reply.status(410).send({ code: 'INVITE_USED' })
    ...
    const result = await execute(
      'INSERT INTO users (email, password_hash, first_name, last_name, role, tenant_id, is_active, locale) VALUES (?, ?, ?, ?, ?, ?, 1, ?)',
      [invite.email, hash, invite.first_name, invite.last_name, invite.role, tenantIds[0] ?? null, invite.locale]
    )
